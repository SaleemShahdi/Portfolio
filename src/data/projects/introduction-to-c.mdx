...
technologies:
  - C
  - Computer Architecture  # Added for consistency
  - Data Structures
  - Algorithms
  - Memory Management
  - Makefiles
...
---
- **Rot13:** A lightweight command-line utility that performs a Rot-13 substitution cipher on a string, demonstrating foundational C programming and character manipulation.
...
---
- **sorta:** A command-line utility that sorts string arguments using an efficient, in-place Insertion Sort algorithm.
...
---
- **balance:** A command-line tool that validates the correct nesting of delimiters using a custom-implemented stack and manual memory management.
...
---
- **list:** An interactive C program that implements a sorted doubly linked list to dynamically manage integers, showcasing advanced pointer manipulation.
...
---
- **mexp:** A command-line utility that calculates matrix exponentiation (M^n) by implementing a recursive algorithm.
...
---
- **bst:** A comprehensive program that implements a complete Binary Search Tree from scratch, supporting full CRUD operations.
...
- Demonstrated meticulous memory management by authoring a `freeAll` function that uses a recursive post-order traversal to deallocate every node.
---
